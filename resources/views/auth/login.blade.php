    ...
    <div class="language-selector-login-wrapper">
      ...
      <div class="language-selector-login-trigger" id="languageTriggerLogin">
        <span class="flag-display" id="flagDisplayLogin">
          <img
            src="{{ asset('public/images/flags/' . (app()->getLocale() == 'es' ? 'colombia' : (app()->getLocale() == 'en' ? 'usa' : 'china')) . '.png') }}"
            alt="Flag" />
        </span>
        <span
  ...
  <div class="login-container">
    ...
    <form method="POST" action="{{ route('login') }}" autocomplete="off">
      ...
      <div class="input-group">
        ...
        <input type="password" id="password" name="password" placeholder="{{ __('common.ingresa_tu_contraseña') }}"
          required />
        @error('password')<div class="invalid-feedback">{{ $message }}</div>@enderror
      </div>
      <div class="forgot-password">
        <a href="{{ route('password.request') }}">{{ __('common.olvidaste_contraseña') }}</a>
      </div>
      @if(session('error'))
