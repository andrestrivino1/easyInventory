        ...
        <div class="mx-auto" style="max-width:1400px;">
                ...
                <div
                    ...
                    @if($isAdmin)
                        <form method="GET" action="{{ route('stock.index') }}" id="filter-form"
                            ...
                            <button type="submit"
                                style="background: #0066cc; color: white; border: none; padding: 6px 16px; border-radius: 6px; cursor: pointer; font-weight: 500;">
                                <i class="bi bi-funnel"></i> Filtrar
                            </button>
                            <a href="{{ route('stock.index') }}"
                                style="background: #6c757d; color: white; border: none; padding: 6px 16px; border-radius: 6px; cursor: pointer; font-weight: 500; text-decoration: none;">
                    ...
                    @if($canExport)
                        <div style="display: flex; gap: 10px;">
                            <a href="{{ route('stock.export-pdf', request()->query()) }}" class="btn btn-primary"
                                style="padding: 8px 16px; text-decoration: none; border-radius: 6px; font-weight: 500;">
                                <i class="bi bi-file-earmark-pdf me-2"></i>Descargar PDF
                            </a>
                            <a href="{{ route('stock.export-excel', request()->query()) }}" class="btn btn-success"
                                style="padding: 8px 16px; text-decoration: none; border-radius: 6px; font-weight: 500; background: #28a745; color: white;">
            ...
            <div id="products-section">
                <div class="section-title" style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    @if($canExport)
                        <a href="{{ route('stock.export-excel-products', request()->query()) }}" class="btn btn-sm btn-success"
                            style="padding: 6px 12px; text-decoration: none; border-radius: 6px; font-weight: 500; background: #28a745; color: white; font-size: 12px;">
                ...
                <div class="mb-3" style="max-width: 400px;">
                    <div style="position: relative;">
                        <input type="text" id="search-products-stock" class="form-control" placeholder="Buscar productos..."
                            style="padding-left: 40px; border-radius: 25px; border: 2px solid #e0e0e0;">
                        <i class="bi bi-search"
                            style="position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #999; font-size: 16px;"></i>
                    </div>
                </div>
                                ...
                                <tr @if($bajoStock) style="background-color: #fff3e0; border-left: 4px solid #ff9800;" @endif>
                                    ...
                                    <td>
                                        ...
                                        @if($warehouseIdToShow)
                                            ...
                                            @if($warehouse)
                                                <span
                                                    style="font-weight: 500;">{{ $warehouse->nombre }}{{ $warehouse->ciudad ? ' - ' . $warehouse->ciudad : '' }}</span>
                                            @else
                                    ...
                                    <td>
                                        @if(isset($product->container_reference))
                                            {{-- Si el producto tiene contenedor asignado (puede ser uno o varios unificados),
                                            mostrar todas las referencias --}}
                                            <span style="font-size: 13px;"
                                                title="{{ $product->container_reference }}">{{ $product->container_reference }}</span>
                                        @else
                                            @php
                                    ...
                                    <td>
                                        @if(isset($product->cajas_en_contenedor))
                                            ...
                                            --}}
                                            <strong @if($product->cajas_en_contenedor <= 3)
                                            style="color: #ff9800; font-weight: bold;" @endif>
                                                {{ number_format($product->cajas_en_contenedor, 0) }} cajas
                                            </strong>
                                            @if($product->cajas_en_contenedor <= 3)
                                                <span style="color: #ff9800; font-size: 11px; margin-left: 5px;"
                                                    title="Bajo stock: 3 o menos cajas (mínimo recomendado: 4 cajas)">⚠️</span>
                                            @endif
                                        @else
                                            @if($product->tipo_medida === 'caja' && $cajasReales !== null)
                                                <strong @if($bajoStock) style="color: #ff9800; font-weight: bold;"
                                                @endif>{{ number_format($cajasReales, 0) }} cajas</strong>
                                                @if($bajoStock)
                                                    <span style="color: #ff9800; font-size: 11px; margin-left: 5px;"
                                                        title="Bajo stock: 3 o menos cajas (mínimo recomendado: 4 cajas)">⚠️</span>
                                                @endif
                                    ...
                                    <td>
                                        ...
                                        @else
                                            ...
                                            @if($product->id == 57)
                                                <br><span style="color:red; font-size:10px;">DEBUG: Stock={{ $stockEnBodega }} |
                                                    HasCont={{ isset($product->container_reference) ? 'YES' : 'NO' }}</span>
                                            @endif
                                        @endif
                                    </td>
                ...
                <div id="products-pagination">
                    @if(method_exists($products, 'total') && $products->total() > $products->perPage())
                        <div
                            style="padding: 20px; background: white; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.08); margin-top: 20px;">
                            <div
                                style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                                <div style="color: #666; font-size: 14px;">
                                    Mostrando {{ $products->firstItem() ?? 0 }} - {{ $products->lastItem() ?? 0 }} de
                    ...
                    @elseif(method_exists($products, 'total') && $products->total() > 0)
                        <div
                            style="padding: 15px; background: white; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.08); margin-top: 20px; text-align: center; color: #666; font-size: 14px;">
            ...
            @if(!$selectedWarehouseId || in_array($selectedWarehouseId, $bodegasQueRecibenIds))
                <div id="containers-section">
                    <div class="section-title" style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        @if($canExport)
                            <a href="{{ route('stock.export-excel-containers', request()->query()) }}"
                    ...
                    <div class="mb-3" style="max-width: 400px;">
                        <div style="position: relative;">
                            <input type="text" id="search-containers" class="form-control" placeholder="Buscar contenedores..."
                                style="padding-left: 40px; border-radius: 25px; border: 2px solid #e0e0e0;">
                            <i class="bi bi-search"
                                style="position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #999; font-size: 16px;"></i>
                        </div>
            ...
            <div id="transfers-section">
                <div class="section-title" style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    @if($canExport)
                        <a href="{{ route('stock.export-excel-transfers', request()->query()) }}" class="btn btn-sm btn-success"
                            style="padding: 6px 12px; text-decoration: none; border-radius: 6px; font-weight: 500; background: #28a745; color: white; font-size: 12px;">
                            <i class="bi bi-file-earmark-excel me-1"></i>Excel
                        </a>
                    @endif
                ...
                <form method="GET" action="{{ route('stock.index') }}" id="transfers-date-filter"
                    style="margin-bottom: 15px; display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <input type="date" name="transfers_date_from" id="transfers_date_from"
                            value="{{ request('transfers_date_from') }}"
                            style="padding: 6px 12px; border: 1px solid #ddd; border-radius: 6px;">
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <label for="transfers_date_to" style="font-weight: 500; color: #333; margin: 0;">Hasta:</label>
                        <input type="date" name="transfers_date_to" id="transfers_date_to"
                            value="{{ request('transfers_date_to') }}"
                            style="padding: 6px 12px; border: 1px solid #ddd; border-radius: 6px;">
                    </div>
                    <button type="submit"
                        style="background: #0066cc; color: white; border: none; padding: 6px 16px; border-radius: 6px; cursor: pointer; font-weight: 500; font-size: 13px;">
                        <i class="bi bi-funnel"></i> Filtrar
                    </button>
                    @if(request('transfers_date_from') || request('transfers_date_to'))
                        <a href="{{ route('stock.index', array_merge(request()->except(['transfers_date_from', 'transfers_date_to', 'transfers_page']))) }}"
                            style="background: #6c757d; color: white; border: none; padding: 6px 16px; border-radius: 6px; cursor: pointer; font-weight: 500; text-decoration: none; font-size: 13px;">
                            <i class="bi bi-x-circle"></i> Limpiar
                        </a>
                    @endif
                </form>
                                    ...
                                    <td>
                                        @foreach($transfer->products as $prod)
                                            <div style="font-size: 13px; margin-bottom: 4px;">
                                                ...
                                                <span style="color: #666;">({{ $prod->pivot->quantity }}
                                                    {{ $prod->tipo_medida === 'caja' ? 'cajas' : 'unidades' }})</span>
                                            </div>
                                        @endforeach
                                    </td>
            ...
            <div id="salidas-section">
                <div class="section-title" style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    @if($canExport)
                        <a href="{{ route('stock.export-excel-salidas', request()->query()) }}" class="btn btn-sm btn-success"
                            style="padding: 6px 12px; text-decoration: none; border-radius: 6px; font-weight: 500; background: #28a745; color: white; font-size: 12px;">
                            <i class="bi bi-file-earmark-excel me-1"></i>Excel
                        </a>
                    @endif
                ...
                <form method="GET" action="{{ route('stock.index') }}" id="salidas-date-filter"
                    style="margin-bottom: 15px; display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <input type="date" name="salidas_date_from" id="salidas_date_from"
                            value="{{ request('salidas_date_from') }}"
                            style="padding: 6px 12px; border: 1px solid #ddd; border-radius: 6px;">
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <label for="salidas_date_to" style="font-weight: 500; color: #333; margin: 0;">Hasta:</label>
                        <input type="date" name="salidas_date_to" id="salidas_date_to"
                            value="{{ request('salidas_date_to') }}"
                            style="padding: 6px 12px; border: 1px solid #ddd; border-radius: 6px;">
                    </div>
                    <button type="submit"
                        style="background: #0066cc; color: white; border: none; padding: 6px 16px; border-radius: 6px; cursor: pointer; font-weight: 500; font-size: 13px;">
                    ...
                    @if(request('salidas_date_from') || request('salidas_date_to'))
                        <a href="{{ route('stock.index', array_merge(request()->except(['salidas_date_from', 'salidas_date_to', 'salidas_page']))) }}"
                            style="background: #6c757d; color: white; border: none; padding: 6px 16px; border-radius: 6px; cursor: pointer; font-weight: 500; text-decoration: none; font-size: 13px;">
                            <i class="bi bi-x-circle"></i> Limpiar
                        </a>
                    @endif
                ...
                <div class="mb-3" style="max-width: 400px;">
                    <div style="position: relative;">
                        <input type="text" id="search-salidas" class="form-control" placeholder="Buscar salidas..."
                            style="padding-left: 40px; border-radius: 25px; border: 2px solid #e0e0e0;">
                        <i class="bi bi-search"
                            style="position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #999; font-size: 16px;"></i>
                    </div>
