  ...
  <div class="form-container">
    ...
    <form action="{{ route('products.store') }}" method="POST" autocomplete="off">
      ...
      <div id="box-fields" style="display: none;">
        ...
        <input type="number" id="stock_cajas" name="stock_cajas" min="1" value="{{ old('stock_cajas', 1) }}" oninput="calculateTotalStock()">
      ...
      <select name="container_id" id="container_id" required>
        ...
        @foreach($containers as $cont)
            <option value="{{ $cont->id }}" {{ old('container_id') == $cont->id ? 'selected' : '' }}>{{ $cont->reference }}</option>
        @endforeach
      </select>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Forzar tipo_medida a 'caja' justo antes del submit para Buenaventura
  if(form && wh && tipoSelect){
