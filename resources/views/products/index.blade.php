            ...
            @if(isset($canCreateProducts) && $canCreateProducts)
                <div class="d-flex justify-content-end align-items-center mb-3" style="gap:10px;">
                    <a href="{{ route('products.create') }}" class="btn btn-primary rounded-pill px-4"
                        style="font-weight:500;"><i class="bi bi-plus-circle me-2"></i>Nuevo Producto</a>
                </div>
            @endif
            <h2 class="mb-4" style="text-align:center;color:#333;font-weight:bold">Inventario de Productos</h2>

            <!-- Campo de búsqueda server-side -->
            <div class="mb-3" style="max-width: 480px; margin: 0 auto 20px; text-align: center;">
                <div style="position: relative; width: 100%;">
                    <input type="text" id="search-products" class="form-control"
                        placeholder="Buscar por nombre, código o medidas..." value="{{ $search }}"
                        style="padding-left: 40px; padding-right: 38px; border-radius: 25px; border: 2px solid #e0e0e0; width: 100%;">
                    <i class="bi bi-search"
                        style="position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #999; font-size: 16px; pointer-events: none;"></i>
                    @if($search)
                        <a href="{{ route('products.index') }}" title="Limpiar búsqueda"
                            style="position: absolute; right: 14px; top: 50%; transform: translateY(-50%); color: #999; font-size: 18px; text-decoration: none; line-height:1;">&times;</a>
                    @endif
                </div>
                @if($search)
                    <div style="font-size:13px; color:#666; margin-top:6px;">
                        {{ $productos->total() }} resultado(s) para <strong>"{{ $search }}"</strong>
                    </div>
                @endif
            </div>
                ...
                <tbody>
                    @forelse($productos as $producto)
                        <tr>
                            ...
                            <td>{{ $producto->peso_empaque !== null ? number_format($producto->peso_empaque, 2, ',', '.') : '-' }}
                            </td>
                            <td>
                                @if($producto->estado)
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (searchInput) {
            ...
            // Foco al final del texto si hay búsqueda activa
