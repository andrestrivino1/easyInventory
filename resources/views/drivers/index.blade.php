  ...
  <div class="container-fluid" style="padding-top:32px; min-height:88vh;">
    <div class="mx-auto" style="max-width:1050px;">
      <div class="d-flex justify-content-end align-items-center mb-3" style="gap:10px;">
        <a href="{{ route('drivers.create') }}" class="btn btn-primary rounded-pill px-4" style="font-weight:500;"><i
            class="bi bi-plus-circle me-2"></i>Nuevo conductor</a>
      </div>
      ...
      <div class="mb-3" style="max-width: 400px; margin: 0 auto 20px; text-align: center;">
        <div style="position: relative; width: 100%;">
          <input type="text" id="search-drivers" class="form-control" placeholder="Buscar conductores..."
            ...
            <tr>
              <td class="photo-container">
                @if($driver->photo_path)
                  <img src="{{ route('drivers.photo', $driver) }}" alt="Foto de {{ $driver->name }}" class="driver-photo"
                    onclick="window.open('{{ route('drivers.photo', $driver) }}', '_blank')">
                @else
              ...
              <td class="photo-container">
                @if($driver->vehicle_photo_path)
                  <img src="{{ route('drivers.vehicle-photo', $driver) }}" alt="Foto vehículo {{ $driver->vehicle_plate }}"
                    class="vehicle-photo" onclick="window.open('{{ route('drivers.vehicle-photo', $driver) }}', '_blank')">
                @else
                  <div class="photo-placeholder">Sin foto</div>
                @endif
              </td>
              ...
              <td>
                @if($driver->social_security_date)
                  ...
                  @endphp
                  <span
                    style="color: {{ $isExpired ? '#dc3545' : '#333' }}; font-weight: {{ $isExpired ? 'bold' : 'normal' }};">
                    {{ $securityDate->format('d/m/Y') }}
                  </span>
                  @if($driver->social_security_pdf)
                    <a href="{{ route('drivers.social-security-pdf', $driver) }}" target="_blank"
                      style="margin-left: 5px; color: #dc3545;" title="Ver PDF">
      ...
      <!-- Paginación -->
      <div id="drivers-pagination" class="mt-4">
        @if(method_exists($drivers, 'total') && $drivers->total() > $drivers->perPage())
          <div style="padding: 20px; background: white; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.08);">
        ...
        @elseif(method_exists($drivers, 'total') && $drivers->total() > 0)
          <div
            ...
            Mostrando {{ $drivers->total() }} conductor(es)
          </div>
        @endif
      </div>
    </div>
