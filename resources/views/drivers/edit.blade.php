    ...
    <div class="form-bg">
        <div class="form-container">
            ...
            <form method="POST" action="{{ route('drivers.update', $driver) }}" autocomplete="off"
                ...
                <input name="identity" type="text" class="form-control" required maxlength="20"
                    value="{{ old('identity', $driver->identity) }}">
                @error('identity') <div class="invalid-feedback">{{ $message }}</div>@enderror

                <label for="phone">Teléfono</label>
                <input name="phone" type="text" class="form-control" maxlength="20"
                ...
                @if($driver->photo_path)
                    <div style="margin-bottom: 8px;">
                        <img src="{{ Storage::url($driver->photo_path) }}" alt="Foto del conductor"
                ...
                <input name="vehicle_plate" type="text" class="form-control" required maxlength="20"
                ...
                @if($driver->vehicle_photo_path)
                    <div style="margin-bottom: 8px;">
                        <img src="{{ Storage::url($driver->vehicle_photo_path) }}" alt="Foto del vehículo"
                            style="max-width: 150px; max-height: 150px; border: 1px solid #ddd; border-radius: 4px;">
                    </div>
                @endif
                ...
                <input name="social_security_date" type="date" class="form-control"
                    value="{{ old('social_security_date', $driver->social_security_date ? \Carbon\Carbon::parse($driver->social_security_date)->format('Y-m-d') : '') }}">
                ...
                @if($driver->social_security_pdf)
                    <div style="margin-bottom: 8px;">
                        <a href="{{ route('drivers.social-security-pdf', $driver) }}" target="_blank"
                            style="color: #4a8af4; text-decoration: underline;">
                ...
                <input name="vehicle_owner" type="text" class="form-control" maxlength="255"
                    value="{{ old('vehicle_owner', $driver->vehicle_owner) }}">
                @error('vehicle_owner') <div class="invalid-feedback">{{ $message }}</div>@enderror

                <label for="capacity">Capacidad de Carga (kg)</label>
                <input name="capacity" type="number" step="0.01" class="form-control" min="0"
