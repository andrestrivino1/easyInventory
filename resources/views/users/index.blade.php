    ...
    <div class="container-fluid" style="padding-top:32px; padding-bottom:40px; min-height:88vh;">
        <div class="mx-auto" style="max-width:1050px;">
            @if(auth()->user()->rol === 'admin')
                <div class="d-flex justify-content-end align-items-center mb-3"><a href="{{ route('users.create') }}"
                        class="btn btn-primary rounded-pill px-4" style="font-weight:500;"><i
                            class="bi bi-person-plus me-2"></i>Nuevo usuario</a></div>
            @endif
            ...
            <div class="mb-4" style="max-width: 400px; margin: 0 auto 24px; text-align: center;">
                <div style="position: relative; width: 100%;">
                    <input type="text" id="search-users" class="form-control" placeholder="Buscar usuarios..."
            ...
            <table class="table-users" id="users-table">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-muted py-5"><i class="bi bi-people text-secondary"
                                    style="font-size:2.2em;"></i><br>
                                <div class="mt-2">No hay usuarios registrados.</div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div id="users-pagination" class="mt-4">
                @if(method_exists($usuarios, 'total') && $usuarios->total() > $usuarios->perPage())
                ...
                @elseif(method_exists($usuarios, 'total') && $usuarios->total() > 0)
                    <div style="padding: 15px; background: white; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.08); text-align: center; color: #666; font-size: 14px;">
                        Mostrando {{ $usuarios->total() }} usuario(s)
                    </div>
                @endif
            </div>
        </div>
