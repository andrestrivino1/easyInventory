...
<div class="form-container">
    ...
    <form method="POST" action="{{ route('containers.update', $container) }}" autocomplete="off">
        @csrf
        @method('PUT')
        ...
        <input name="product_name" type="text" required value="{{ old('product_name', $container->product_name) }}">
        @error('product_name') <div class="invalid-feedback">{{ $message }}</div>@enderror

        <label for="boxes">Cantidad de cajas*</label>
        <input name="boxes" id="boxes" type="number" required min="0" value="{{ old('boxes', $container->boxes) }}" oninput="calcSheets()">
        @error('boxes') <div class="invalid-feedback">{{ $message }}</div>@enderror

        <label for="sheets_per_box">Láminas por caja*</label>
        <input name="sheets_per_box" id="sheets_per_box" type="number" required min="1" value="{{ old('sheets_per_box', $container->sheets_per_box) }}" oninput="calcSheets()">
        ...
        <textarea name="note" rows="2">{{ old('note', $container->note) }}</textarea>
