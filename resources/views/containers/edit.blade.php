...
<div class="form-container">
    ...
    <form method="POST" action="{{ route('containers.update', $container) }}" autocomplete="off" id="containerForm">
        ...
        <input name="reference" type="text" required value="{{ old('reference', $container->reference) }}" placeholder="Ej: CONT-001">
        @error('reference') <div class="invalid-feedback">{{ $message }}</div>@enderror

        <label for="note">Observación</label>
        <textarea name="note" rows="2" placeholder="Notas adicionales sobre el contenedor">{{ old('note', $container->note) }}</textarea>
...
function addProduct(existingProduct = null) {
    ...
    const selectedBoxes = existingProduct ? existingProduct.pivot.boxes : 0;
        ...
        <div class="product-fields">
            <div>
                ...
                <select name="products[${productIndex}][product_id]" required onchange="updateProductFields(${productIndex})">
                    ...
                    ${products.map(p => `<option value="${p.id}" data-tipo="${p.tipo_medida}" data-medidas="${p.medidas || ''}" ${p.id == selectedProductId ? 'selected' : ''}>${p.nombre} (${p.codigo})</option>`).join('')}
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="products[${productIndex}][boxes]" min="0" value="${selectedBoxes}" required oninput="calculateSheets(${productIndex})">
            </div>
            <div>
                <label for="products[${productIndex}][sheets_per_box]">Láminas por caja*</label>
                <input type="number" name="products[${productIndex}][sheets_per_box]" min="1" value="${selectedSheets}" required oninput="calculateSheets(${productIndex})">
            </div>
            <div>
