...
@section('content')
        ...
        <div class="form-container">
            ...
            <form method="POST" action="{{ route('salidas.store') }}" autocomplete="off" id="salidaForm">
                ...
                @if($isAdmin || ($isCliente && $warehouses->count() > 1) || ($isFuncionario && $warehouses->count() > 1))
                    ...
                    <select name="warehouse_id" id="warehouse_id" required
                        ...
                        @foreach($warehouses as $wh)
                            <option value="{{ $wh->id }}"
                                data-is-buenaventura="{{ in_array($wh->id, $bodegasBuenaventuraIds) ? 'true' : 'false' }}" {{ old('warehouse_id') == $wh->id ? 'selected' : '' }}>
                ...
                @elseif($isCliente && $warehouses->count() == 1)
                    ...
                    <label>Bodega</label>
                    <input type="text"
                        value="{{ $warehouses->first()->nombre }}{{ $warehouses->first()->ciudad ? ' - ' . $warehouses->first()->ciudad : '' }}"
                ...
                @else
                    ...
                    <label>Bodega</label>
                    <input type="text" value="{{ $user->almacen->nombre ?? 'N/A' }}" readonly
                        style="background:#e9ecef; pointer-events:none;">
                @endif
                ...
                <input type="text" name="a_nombre_de" id="a_nombre_de" value="{{ old('a_nombre_de') }}"
                    placeholder="Nombre del cliente" required>
                @error('a_nombre_de') <div class="invalid-feedback">{{ $message }}</div>@enderror

                <label for="nit_cedula">NIT/Cédula*</label>
                <input type="text" name="nit_cedula" id="nit_cedula" value="{{ old('nit_cedula') }}"
                ...
                <input type="text" name="aprobo" id="aprobo" value="{{ old('aprobo') }}"
                ...
                <input type="text" name="ciudad_destino" id="ciudad_destino" value="{{ old('ciudad_destino') }}"
                ...
                <div id="driver-external-block" style="display: none;">
                    ...
                    <input type="text" name="external_driver_name" id="external_driver_name"
                        value="{{ old('external_driver_name') }}" maxlength="255" placeholder="Nombre completo">
                    ...
                    <input type="text" name="external_driver_identity" id="external_driver_identity"
                        value="{{ old('external_driver_identity') }}" maxlength="50" placeholder="Documento de identidad">
                    ...
                    <input type="text" name="external_driver_plate" id="external_driver_plate"
                        value="{{ old('external_driver_plate') }}" maxlength="50" placeholder="Placa">
                    ...
                    <input type="text" name="external_driver_phone" id="external_driver_phone"
                        value="{{ old('external_driver_phone') }}" maxlength="20" placeholder="Número de teléfono">
    ...
    <script>
        ...
        function buildProductOptionText(product) {
            ...
            // Mostrar referencia del contenedor si existe
            if (product.containers && product.containers.length === 1) {
                ...
                // Mostrar todos los contenedores separados por coma
                const refs = product.containers.map(c => c.reference).join(', ');
                text += ` [${refs}]`;
            }
            return text;
        }
        ...
        function updateProductSelects() {
            ...
            productSelects.forEach(select => {
                ...
                // Disparar evento change para actualizar selector de contenedor e info de stock
                if (select.value) {
                    updateContainerForProduct(parseInt(index));
                } else {
                    // Si no hay producto seleccionado, ocultar el bloque de contenedor
                    const containerBlock = document.getElementById(`container-block-${index}`);
                    if (containerBlock) containerBlock.style.display = 'none';
                    updateProductInfo(parseInt(index));
                }
            });
        }
            ...
            productItem.innerHTML = `
                        ...
                        <div class="product-fields">
                            <div class="full-width">
                                ...
                                <select name="products[${productIndex}][product_id]" id="product-select-${productIndex}" required onchange="updateContainerForProduct(${productIndex}); updateProductInfo(${productIndex})">
        ...
        function updateContainerForProduct(index) {
            ...
            let containers = [];
            try {
                containers = JSON.parse(selectedOption.getAttribute('data-containers') || '[]');
            } catch (e) { containers = []; }

            containerSelect.innerHTML = '<option value="">Seleccione un contenedor</option>';

            if (containers.length === 0) {
                // Sin contenedores específicos: campo oculto
                containerBlock.style.display = 'none';
                containerSelect.removeAttribute('required');
            } else if (containers.length === 1) {
                // Solo un contenedor: auto-seleccionar y mostrar como info
                ...
                // Múltiples contenedores: mostrar dropdown para elegir
                containers.forEach(c => {
                    const opt = document.createElement('option');
                    opt.value = c.id;
                    opt.textContent = c.reference;
                    opt.setAttribute('data-container-id', c.id);
                    containerSelect.appendChild(opt);
                });
                containerBlock.style.display = 'block';
                containerSelect.setAttribute('required', 'required');
        ...
        function updateProductInfo(index) {
            ...
            if (selectedOption && selectedOption.value) {
                // Stock base total del producto
                let stock = parseInt(selectedOption.getAttribute('data-stock')) || 0;

                // Si hay contenedor seleccionado y el producto tiene stock por contenedor, filtrar
                ...
                if (selectedContainerId) {
                    ...
                    if (productData && productData.containers && productData.containers.length > 1) {
                        ...
                        if (foundContainer && foundContainer.stock !== undefined) {
                            stock = foundContainer.stock;
                        }
                    }
                }
